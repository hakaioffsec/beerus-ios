...
final class FridaVersionsViewController: UIViewController {
    ...
    @objc private func uninstallTapped() {
        let alert = UIAlertController(
            ...
            message: "This will stop frida-server and remove the binary from /usr/sbin/frida-server. Continue?",
    ...
    private func performUninstall() {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    // Clear any installed status in cells
    ...
    private func installRelease(at index: Int) {
        ...
        guard RootExec.isAvailable else {
            showAlert(title: "Daemon Offline", message: "The beerus daemon is not running. Install it first.")
            return
        }
    ...
    private func installBinary(from filePath: String, at index: Int) {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            DispatchQueue.main.async {
                if success {
                    // Clear previous installed status
...
extension FridaVersionsViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(
            withIdentifier: FridaReleaseCell.identifier,
            for: indexPath
        ) as? FridaReleaseCell else {
...
extension FridaVersionsViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        guard let asset = release.fridaServerAsset(for: arch) else {
            showAlert(title: "Not Available", message: "No frida-server package for \(arch) in \(release.version).")
            return
        }
