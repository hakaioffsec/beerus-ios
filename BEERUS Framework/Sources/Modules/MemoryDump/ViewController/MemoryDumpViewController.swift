...
final class MemoryDumpViewController: BaseViewController {
    ...
    private lazy var titleLabel = UILabel.styled(
        text: "Memory Dump", font: AppFont.bold(20), alignment: .center
    )
    ...
    override func viewDidLoad() {
        ...
        nc.addObserver(self, selector: #selector(checkFridaStatus),
                       name: UIApplication.didBecomeActiveNotification, object: nil)
        nc.addObserver(self, selector: #selector(fridaStatusDidChange),
                       name: FridaChecker.statusDidChangeNotification, object: nil)
    }
    ...
    private func updateUI(fridaRunning: Bool) {
        ...
        dumpButton.setTitle(fridaRunning ? "Dump Memory" : "Frida OFF", for: .normal)
        dumpButton.isEnabled = fridaRunning
        dumpButton.alpha = fridaRunning ? 1.0 : 0.5
    }

    @objc private func dumpButtonTapped() {
        let processListVC = ProcessListViewController()
        processListVC.menuDelegate = menuDelegate
        navigationController?.pushViewController(processListVC, animated: true)
    }
}

extension MemoryDumpViewController: ViewCode {
